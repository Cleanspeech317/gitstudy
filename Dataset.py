...
def data_load(dataset, has_v=True, has_a=True, has_t=True):
    ...
    if dataset == 'movielens':
        num_user = 55485
        num_item = 5986
        v_feat = np.load(dir_str+'/Feature_v.npy', allow_pickle=True) if has_v else None
        a_feat = np.load(dir_str+'/Feature_a.npy', allow_pickle=True) if has_a else None
        t_feat = np.load(dir_str+'/Feature_tl.npy', allow_pickle=True) if has_t else None
