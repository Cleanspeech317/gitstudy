...
from BaseModel_MMGCN import BaseModel
...
class MMGCN(torch.nn.Module):
    def __init__(self, v_feat, a_feat, t_feat, edge_index, batch_size, num_user, num_item, aggr_mode, concate, num_layer, has_id, reg_weight, dim_x,user_item_dict=None):
    ...
    def accuracy(self, dataset, topk=10, neg_num=1000):
        ...
        for data in dataset:
            bar.update(1)
            if len(data) < 1002:#一个用户id，500个负id和1和正id
                continue
            #意义何在
            sum_item += 1
            user = data[0]
            ...
            #从结果这个(num_user + num_item, dim_x)大小的数组中根据索引取出结果切入，计算什么loss
